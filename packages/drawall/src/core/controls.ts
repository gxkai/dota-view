...
import { Amount, Unit, UNITLESS, Units } from "@/core/units";
import { Angle, Distance, Position } from "@/core/spaces";
import { Degrees, lerp, Point, toDegrees, toRadians } from "@/core/linalg";
import { DefaultMap, impossible, Kinds, Ref, RefK, Refs, RefView } from "@/core/util";
import { unwrap } from "@/core/lib/minewt";
import { Popup } from "@/core/popups";
import { App } from "@/core/app";

export class ElementWrap<E extends HTMLElement> {
...
export class IconButton extends ElementWrap<HTMLElement> {
...
export type AutoFieldListener = (field: AutoField) => void;

export interface AutoFieldUi<T> {
...
export interface AutoFieldHandle<V> {
  readonly value: Ref<V>;
  readonly enabled: RefView<boolean, RefK>;
  readonly hidden: RefView<boolean, RefK>;
}

export type AutoFieldId = Pick<AutoField, 'name' | 'kind'>;

export type AutoFieldValue<A extends AutoField> = [A] extends [{ value: infer V }] ? V : never;

// type puns type puns
export type Amoral<V> = Exclude<V, 'value'>;
...
export type AutoField = AutoFieldNumber
  | AutoFieldAmount 
  | AutoFieldDistance
  | AutoFieldToggle
  | AutoFieldSlider
...
export interface AutoFieldBase<V> {
  name: string;
  label?: string;
  value: Ref<V>;
...
export interface AutoFieldNumber extends AutoFieldBase<number> {
  kind: 'number';
  min?: number;
  max?: number;
}

export interface AutoFieldSlider extends AutoFieldBase<number> {
  kind: 'slider';
  min: number;
  max: number;
}

export interface AutoFieldAngle extends AutoFieldBase<Angle> {
  kind: 'angle';
}

export interface AutoFieldText extends AutoFieldBase<string> {
  kind: 'text';
}

export interface AutoFieldAmount extends AutoFieldBase<Amount> {
  kind: 'amount';
  unit: Units;
  min?: Amount;
  max?: Amount;
...
export interface AutoFieldDistance extends AutoFieldBase<Distance> {
  kind: 'distance';
  min?: Distance;
  max?: Distance;
}

export interface AutoFieldSelect<T extends string> extends AutoFieldBase<T> {
  kind: 'select';
  items: SelectItem<T>[];
}
...
export interface AutoFieldToggle extends AutoFieldBase<boolean> {
  kind: 'toggle';
  icons?: ToggleIcons;
}

export interface AutoFieldSeparator extends AutoFieldBase<'separator'> {
  kind: 'separator',
}

export interface AutoFieldButton extends AutoFieldBase<'button'> {
  kind: 'button';
  icon?: URL | null;
  onClick: () => void;
}

export type FlowOrientation = 'row' | 'column';
...
export class MiniForm extends ElementWrap<HTMLElement> implements Resetable {
...
export class Separator extends ElementWrap<HTMLElement> {
...
export class MiniLabel extends ElementWrap<HTMLElement> {
...
export interface InputParseValue<V> { value: V; };
export interface InputParseError { error: string; };
export type InputParse<V> = InputParseValue<V> | InputParseError;

export abstract class MiniFormInput<V, E extends HTMLElement> extends ElementWrap<E> implements Resetable {
...
export class AmountInput extends MiniFormInput<Amount, HTMLInputElement> {
...
export class AngleInput extends MiniFormInput<Angle, HTMLInputElement> {
...
export class NumberInput extends MiniFormInput<number, HTMLInputElement> {
  constructor(
...
export class TextInput extends MiniFormInput<string, HTMLElement> {
