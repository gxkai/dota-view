...
import {
  ...
  Degrees, Edge,
  formatDegrees,
  Frame, mapAngle, normalizeRadians,
  Point,
  Radians, Sign,
  signum,
  TAU,
...
import { Memo } from "@/core/util";

export type SpaceName = 'screen' | 'model' | 'identity';
...
export type CoordinateTransform<V> = (cs: Space, v: V) => V;

// thx to kanwren for help w typing witchcraft 💓
export type SpaceValues<T extends readonly unknown[]> = T extends readonly []
  ? readonly []
  : T extends readonly [infer Start, ...infer End]
    ? readonly [Spaced<Start>, ...SpaceValues<End>]
    : never;

export interface Spaced<V> {
  get: (space: SpaceName) => V;
  space: SpaceName;
}

export abstract class BaseSpaceValue<V> implements Spaced<V> {
...
export class SpaceValue<V> implements Spaced<V> {
  constructor(
...
export type DistanceDivisor = number | Spaced<number>;

export type DivReturnType<Divisor extends DistanceDivisor> = [Divisor] extends [number]
  ? SpaceDistance : number;

export class SpaceDistance extends BaseSpaceValue<number> {
...
export type Distance = SpaceDistance;
export const Distance = SpaceDistance.of;
export const Distances = {
  between: SpaceDistance.between,
  zero: (space: SpaceName) => Distance(0, space),
};

export class SpaceAngle extends BaseSpaceValue<Radians> {
...
export type Angle = SpaceAngle;
export const Angle = SpaceAngle.of;
export const Angles = {
...
export class SpaceVec extends BaseSpaceValue<Vec> {
...
export type Vector = SpaceVec;
export const Vector = SpaceVec.of;
...
export class SpacePos extends BaseSpaceValue<Point> {
...
export type Position = SpacePos;
export const Position = SpacePos.of;
...
export class HalfPlane extends SDF {
  constructor(
...
export class Polygon extends SDF {
