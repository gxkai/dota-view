...
import { JsonArray, NamedSavedComponent, SavedComponent, SavedEcs, SavedEntity } from "@/core/io/json";
import { Counter, Refs } from "@/core/util";
import { App } from "@/core/app";

export type Eid = Newtype<number, { readonly _: unique symbol; }>;
export const Eid = newtype<Eid>();

export type Cid = Newtype<number, { readonly _: unique symbol; }>;
export const Cid = newtype<Cid>();

export const SOLO: unique symbol = Symbol();
...
export class TestComponent extends Component implements Solo {
  readonly [SOLO] = true;
}

export type ComponentType<C extends Component> = new (entity: Entity, ...args: any[]) => C;
...
export type EntityRef<T> = EntityRefImpl<T>;
export const EntityRef = <T, E extends readonly Entity[]>(
  getter: (...entities: E) => T, ...entities: E): EntityRefImpl<T> => {
  return new EntityRefImpl(() => getter(...entities), entities);
};

export type ComponentInflateResult = Component | 'not ready' | 'skip';

export type ComponentFactory<A extends JsonArray>
...
export type System = (ecs: EntityComponentSystem) => void;
