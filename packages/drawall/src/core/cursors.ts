import { Vec } from "@/core/linalg";
import { Vector } from "@/core/spaces";

export type ResizeCursor = 'n-resize' | 's-resize' | 'w-resize' | 'e-resize'
  | 'ne-resize' | 'nw-resize' | 'se-resize' | 'sw-resize'
  | 'ew-resize' | 'ns-resize' | 'nesw-resize' | 'nwse-resize';

export type CursorBuiltin = 'default' | 'none' | 'help' | 'context-menu'
...
export type CursorCustom = `url('${string}')` | `url('${string}') ${number} ${number}`;

export type CursorSingle = CursorBuiltin | CursorCustom;

export type CursorWithFallback = `${CursorSingle}, ${CursorSingle}`;

export type Cursor = CursorSingle | CursorWithFallback;

export const getResizeCursor = (direction: Vector, bidirectional: boolean = true): ResizeCursor => {
