import { LengthConstraint } from "@/core/constraints";
import { UiEventDispatcher } from "@/core/uievents";
import { areEq, impossible, Ref, Refs, RoRef } from "@/core/util";
...
import { Distance, Position, SpaceEdge, Vector, Vectors } from "@/core/spaces";
import { MemoEdge, PhysEdge, PhysNode, PointMass } from "@/core/kinematics";
import { clamp01, Vec } from "@/core/linalg";
...
export type RulerAttachment = RulerAttachmentCanvas | RulerAttachmentVertex | RulerAttachmentEdge;

export interface RulerAttachmentCanvas {
  kind: 'canvas';
  node: PhysNode;
}

export interface RulerAttachmentVertex {
  kind: 'vertex';
  node: PhysNode;
}

export interface RulerAttachmentEdge {
  kind: 'edge',
  node: PhysEdge;
  at?: number;
}

export class RulerEndpoint extends Component implements PointMass, Solo {
...
export class Ruler extends Component implements Solo {
...
export const RulerRenderer = (ecs: EntityComponentSystem) => {
